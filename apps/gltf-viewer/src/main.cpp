...
int main(int argc, char **argv) {
    ...
    args::Command info {
            commands, "info", "Display info about OpenGL",
            [&](args::Subparser &parser) {
                parser.Parse();
                GLFWHandle handle { 1, 1, "" };
                printGLVersion();
            }};
    args::Command interactive {
            commands, "viewer", "Run glTF viewer", [&](args::Subparser &parser) {
                ...
                args::ValueFlag<uint32_t> imageWidth {
                        parser, "width",
                        "Width of window or output image if -b is specified",
                        { "w", "width" }};
                args::ValueFlag<uint32_t> imageHeight {
                        parser, "height",
                ...
                std::vector<GLfloat> lookatParams;
                if (lookat) {
                ...
                viewer::ViewerApplication app {
                        fs::path { argv[0] }, width, height, args::get(file),
