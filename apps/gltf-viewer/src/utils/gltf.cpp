...
glm::mat4 getLocalToWorldMatrix(const tinygltf::Node &node, const glm::mat4 &parentMatrix) {
    ...
    const auto rotationQuat =
            node.rotation.empty()
            ? glm::quat(1, 0, 0, 0)
            : glm::quat(static_cast<GLfloat>(node.rotation[3]), static_cast<GLfloat>(node.rotation[0]),
                        static_cast<GLfloat>(node.rotation[1]),
                        static_cast<GLfloat>(node.rotation[2])); // prototype is w, x, y, z
...
void computeSceneBounds(const tinygltf::Model &model, glm::vec3 &bboxMin, glm::vec3 &bboxMax) {
    ...
    bboxMin = glm::vec3(std::numeric_limits<GLfloat>::max());
    bboxMax = glm::vec3(std::numeric_limits<GLfloat>::lowest());
                    ...
                    if (node.mesh >= 0) {
                        ...
                        for (size_t pIdx = 0; pIdx < mesh.primitives.size(); ++pIdx) {
                            ...
                            const auto positionByteStride =
                                    positionBufferView.byteStride ? positionBufferView.byteStride : 3 * sizeof(GLfloat);
                            ...
                            if (primitive.indices >= 0) {
                                ...
                                for (size_t i = 0; i < indexAccessor.count; ++i) {
                                    ...
                                    switch (indexAccessor.componentType) {
                                        case TINYGLTF_COMPONENT_TYPE_UNSIGNED_BYTE:
                                            index = *reinterpret_cast<const uint8_t *>(
                                                    &indexBuffer.data[indexByteOffset + indexByteStride * i]
                                            );
                                            break;
                                        case TINYGLTF_COMPONENT_TYPE_UNSIGNED_SHORT:
                                    ...
                                    const auto &localPosition = *reinterpret_cast<const glm::vec3 *>(
                                            &positionBuffer.data[byteOffset + positionByteStride * index]
                                    );
                                ...
                                for (size_t i = 0; i < positionAccessor.count; ++i) {
                                    const auto &localPosition = *reinterpret_cast<const glm::vec3 *>(
                                            &positionBuffer.data[byteOffset + positionByteStride * i]
                                    );
